...
otvf_RecursiveFactorial_factorial = $n => {
    let var$2;
    otvf_RecursiveFactorial_$callClinit();
    otv_StepRecorder_enterMethod($rt_s(0), $rt_s(1));
    otv_StepRecorder_step($rt_s(0), $rt_s(1), 28);
    otv_StepRecorder_captureVar($rt_s(2), $n);
    if ($n <= 1) {
        otv_StepRecorder_step($rt_s(0), $rt_s(1), 29);
        otv_StepRecorder_captureVar($rt_s(2), $n);
        otv_StepRecorder_exitMethod();
        return 1;
    }
    otv_StepRecorder_step($rt_s(0), $rt_s(1), 31);
    otv_StepRecorder_captureVar($rt_s(2), $n);
...
otvf_RecursiveFactorial_main = $args => {
    otvf_RecursiveFactorial_$callClinit();
    otv_StepRecorder_enterMethod($rt_s(0), $rt_s(3));
    otv_StepRecorder_step($rt_s(0), $rt_s(3), 35);
    otv_StepRecorder_captureRef($rt_s(4), $args);
...
jl_Throwable_toString = $this => {
    ...
    if ($message === null)
        ...
        var$3 = jl_StringBuilder__init_();
        jl_StringBuilder_append(jl_StringBuilder_append(var$3, $rt_s(6)), $message);
...
otv_StepRecorder_captureRef = ($name, $value) => {
    ...
    if (otv_StepRecorder_truncated)
        return;
    if ($value === null) {
        otv_StepRecorder_jsCaptureVar($name, $rt_s(8));
        return;
    }
    if (!($value instanceof jl_String) && !($value instanceof jl_Number) && !($value instanceof jl_Boolean) && !($value instanceof jl_Character)) {
        ...
        var$5 = (jl_StringBuilder__init_0($rt_s(9))).$append2(jl_Integer_toUnsignedString($id));
        $sb = ((var$5.$append2($rt_s(10))).$append2(jl_Class_getSimpleName($cls))).$append2($rt_s(11));
        $first = 1;
            ...
            a: {
                ...
                if (!jlr_Modifier_isStatic($f.$getModifiers())) {
                    ...
                    if (!$first)
                        $sb.$append2($rt_s(12));
                    $first = 0;
                    ($sb.$append2($f.$getName())).$append2($rt_s(13));
                    if ($fval === null)
                        $sb.$append2($rt_s(8));
                    else if (!($fval instanceof jl_String) && !($fval instanceof jl_Number) && !($fval instanceof jl_Boolean) && !($fval instanceof jl_Character))
                        ($sb.$append2($rt_s(9))).$append2(jl_Integer_toUnsignedString(jl_System_identityHashCode($fval)));
                    else
                        $sb.$append($fval);
                }
            }
...
otv_StepRecorder_jsStep = ($className, $methodName, $line) => {
    let $sb, $i, var$6;
    otv_StepRecorder_$callClinit();
    $sb = jl_StringBuilder__init_0($rt_s(15));
    $i = 0;
...
otv_StepRecorder_jsCaptureVar = ($name, $value) => {
    ...
    var$3 = jl_System_err();
    var$4 = jl_StringBuilder__init_();
    jl_StringBuilder_append(jl_StringBuilder_append0(jl_StringBuilder_append(jl_StringBuilder_append(var$4, $rt_s(16)), $name), 61), $value);
    var$3.$println0(jl_StringBuilder_toString(var$4));
},
otv_StepRecorder_jsStepLimitReached = () => {
    otv_StepRecorder_$callClinit();
    (jl_System_err()).$println0($rt_s(17));
},
otv_StepRecorder__clinit_ = () => {
...
jlr_Modifier_getModifierNames = () => {
    ...
    if (jlr_Modifier_modifierNames === null)
        jlr_Modifier_modifierNames = $rt_wrapArray(jl_String, [$rt_s(19), $rt_s(20), $rt_s(21), $rt_s(22), $rt_s(23), $rt_s(24), $rt_s(25), $rt_s(26), $rt_s(27), $rt_s(28), $rt_s(29), $rt_s(30)]);
...
jlr_Field_toString = $this => {
    ...
    if ($sb.$length() > 0)
        $sb.$append0(32);
    (((($sb.$append2(jl_Class_getName($this.$getType()))).$append0(32)).$append2(jl_Class_getName($this.$declaringClass))).$append2($rt_s(32))).$append2($this.$getName());
    return $sb.$toString();
},
...
jl_Class_toString = $this => {
    let var$1, var$2, var$3;
    var$1 = jl_Class_isInterface($this) ? $rt_s(33) : !jl_Class_isPrimitive($this) ? $rt_s(34) : $rt_s(5);
...
$rt_stringPool(["org.teavm.visualizer.fixture.RecursiveFactorial", "factorial", "n", "main", "args", "", ": ", "0", "null", "@", ":", "{", ",", "=", "}", "\u0000VIZ:step", "\u0000VIZ:var:", "\u0000VIZ:truncated", "default", "public", "protected", "private", "abstract", "static", "final", "transient", "volatile", "synchronized", "native", "strictfp", "interface", "\n", ".", "interface ", "class ", "[L", "[]"]);
